...
public class ImportExcelUtil {
    ...
    public  List<Map<String,Object>> getBankListByExcel(InputStream in,String fileName) throws Exception{
        ...
        for (int i = 0; i < work.getNumberOfSheets(); i++) {
            ...
            //遍历当前sheet中的所有行 从第二行开始  第一行为数据库列  第二行为中文名称
            colrow =sheet.getRow(0);
            for (int j = sheet.getFirstRowNum()+2; j < sheet.getLastRowNum()+1; j++) {
                ...
                for (int y = row.getFirstCellNum(); y < row.getLastCellNum(); y++) {
                    cell = row.getCell(y);
                    headcell = colrow.getCell(y);
                    li.put(this.getCellValue(headcell).toString(),this.getCellValue(cell));
                }
                list.add(li);
            }
        }
     ...
     * @throws
     */
    public  List<Map<String,Object>> getBankListByExcel2(InputStream in,String fileName) throws Exception{
        List<Map<String,Object>> list = null;
